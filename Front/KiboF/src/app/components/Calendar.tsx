...
import { ChevronLeft, ChevronRight, Plus, Clock, Check, Trash2, Calendar as CalendarIcon, Sparkles, X, Info, CheckCircle2, Circle } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-secondary via-background to-secondary p-4 md:p-8">
      <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            <h1 className="text-foreground flex items-center gap-3">
              ...
              <div>
                ...
                <span className="block text-muted-foreground text-base mt-1">
                  Organiza y gestiona tus actividades
                </span>
              </div>
            </h1>
            <div className="flex items-center gap-4 bg-card px-6 py-3 rounded-2xl shadow-lg border border-border">
              <button
                ...
                className="p-2 hover:bg-secondary rounded-xl transition-all hover:shadow-md"
              >
        ...
        {/* Grid principal: Calendario a la izquierda, Formulario a la derecha */}
        <div className="grid grid-cols-1 lg:grid-cols-[1fr,380px] gap-6">
          ...
          <div className="space-y-6">
            <motion.div
              ...
              className="bg-card rounded-3xl shadow-xl border border-border p-4 md:p-8"
            >
              <div className="grid grid-cols-7 gap-2 md:gap-3 mb-4 md:mb-6">
                {['Dom', 'Lun', 'Mar', 'Mié', 'Jue', 'Vie', 'Sáb'].map((day) => (
                  <div key={day} className="text-center text-muted-foreground p-2 font-semibold text-sm">
                    {day}
                  </div>
                ))}
              </div>
                ...
                {calendarDays.map((day) => {
                  ...
                  return (
                    <motion.button
                      ...
                      whileTap={{ scale: 0.95 }}
                      className={`aspect-square p-3 rounded-2xl border-2 transition-all relative font-medium ${
                        isSelected
                          ? 'bg-gradient-to-br from-primary to-accent text-white border-primary shadow-xl scale-105'
                          : isTodayDate
                          ? 'bg-primary/10 border-primary text-primary shadow-md'
                          : isCurrentMonth
                          ? 'bg-input-background border-border hover:bg-secondary hover:shadow-lg hover:border-primary/30'
              ...
              <div className="mt-8 pt-6 border-t border-border">
                <div className="flex flex-wrap items-center gap-4 text-sm">
                  <div className="flex items-center gap-2">
                    <div className="w-3 h-3 rounded-full bg-gradient-to-br from-primary to-accent shadow-sm"></div>
                    <span className="text-muted-foreground font-medium">Seleccionado</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-3 h-3 rounded-full bg-primary/20 border-2 border-primary shadow-sm"></div>
          ...
          {/* COLUMNA DERECHA: Formulario Nueva Tarea */}
          <motion.div
            initial={{ opacity: 0, x: 20 }}
            animate={{ opacity: 1, x: 0 }}
            ...
            <div className="bg-card rounded-3xl shadow-xl border border-border overflow-hidden">
              <div className="bg-gradient-to-r from-primary to-accent p-4 md:p-6">
                <div className="flex items-center justify-between text-white">
                  <div>
                    <h3 className="text-lg md:text-xl font-bold">Nueva Tarea</h3>
                    {selectedDate && (
                    ...
                    <div>
                      ...
                      <div className={`p-4 rounded-xl border-2 ${
                        ...
                        <p className={`font-medium ${
                          selectedTask.completed ? 'text-green-600' : 'text-muted-foreground'
                        }`}>
